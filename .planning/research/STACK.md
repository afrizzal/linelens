...
**Domain:** Real-time OEE / manufacturing analytics dashboard + MQTT factory-telemetry simulator (TypeScript, dockerized appliance)
**Researched:** 2026-07-22
**Confidence:** HIGH (all versions verified against live npm registry / official docs on 2026-07-22, not training data)

> Scope note: The high-level stack is **decided and not re-litigated here** — TypeScript end-to-end · Next.js (App Router) web + separate ingestion worker + PostgreSQL in one `docker compose up` · Eclipse Mosquitto broker · SSE (not WebSockets). This document pins the **implementation layer** inside those constraints. Domain facts (OEE formulas, Six Big Losses, telemetry standards) come from `docs/00-domain-research.md` and are treated as settled.
...
npm install next@16 react react-dom \
  ...
  zod@4 pino@10 date-fns@4

# UI
npm install -D tailwindcss@4 @tailwindcss/postcss

# Dev / test
npm install -D typescript@7 tsx@4 vitest@4 \
  @types/pg @testcontainers/postgresql@12 \
  @playwright/test@1   # optional (demo smoke)

# No XState, no Recharts, no ORM #2 — see decision notes below.
```

Docker images to pin in `compose.yaml`: `node:24-slim` (build FROM for app+worker), `postgres:18`, `eclipse-mosquitto:2.0.22`.
...
The genuine tension: the OEE engine derives **state intervals from raw events** (window functions: `LAG` to detect transitions, `date_trunc`/`generate_series` for shift buckets) — historically Prisma's weak spot. **Prisma 7 resolves this with TypedSQL:** put those queries in `prisma/sql/`, get generated **type-safe** functions, and keep normal Prisma Client for the relational parts (orders ↔ output ↔ loss events for the DIFOT drill-down). Net: AIDA-reuse velocity + Studio + migrations, *without* dropping to stringly-typed `$queryRaw` for the credibility-critical math.
...
ECharts natively renders **all four** required visuals: OEE **waterfall** (canonical stacked-bar-with-transparent-placeholder example), **Pareto** (bar + cumulative-% line on a second y-axis), **color-coded production timeline / Gantt** (`custom` series `renderItem` — the signature move), and efficient **live tiles** (canvas + `setOption` merge for streaming). One library = one mental model and a smaller total footprint than mixing two.
...
The verified PackML subset is **small and flat** (minimal-compliant ~4: Aborted/Stopped/Idle/Execute, plus a few like Held/Suspended/Complete — *verify against ISA-TR88.00.02-2015 before coding*, per domain research §8). No nesting, no parallel regions, no actor invocation → **XState's power would be dead weight**. A typed discriminated union + table-driven transitions + exhaustive `switch` gives compile-time impossible-state guarantees and is trivially unit-testable — exactly what the OEE-correctness tests need (deterministic transitions).
...
Use a **single simulation clock** advanced on a `setInterval` tick: `simTime += tickMs * speedFactor`. `speedFactor = 1` for the live 60-second demo (watch breakdown cascade); crank it higher to fast-forward a shift for seeding aggregates/DIFOT history. Each machine's state machine advances on `simTime`; sample **configurable MTBF/MTTR distributions** (defaults, not hardcoded claims — per domain research open item) to schedule the next transition.
...
**If the SSE live feed must be exactly-once / replayable:**
...
**If TypeScript 7 (native compiler) trips a dependency's types:**
...
**If you want a live demo URL (stretch):**
...
---
*Stack research for: real-time OEE / manufacturing analytics dashboard + MQTT simulator (LineLens)*
*Researched: 2026-07-22*
